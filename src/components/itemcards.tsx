...
const badgeColors: Record<Item["badge"], string> = {
  "FIXED PRICE": "bg-[#F48C2510] text-[#F48C25]",
...
export default function ItemCard({ item }: ItemCardProps) {
  return (
    <div className="relative z-0 bg-white border border-[#F48C2520] rounded-2xl overflow-hidden hover:shadow-md transition-shadow cursor-pointer font-[Inter]">
      <div className="h-40 bg-[#f5f0e8] flex items-center justify-center relative">
        <span className="text-slate-300 text-sm">No Image</span>
        <span className={`absolute top-2 left-2 text-[10px] font-bold uppercase px-2 py-0.5 rounded-md ${badgeColors[item.badge]}`}>
          {item.badge}
        </span>
      </div>
